...
|---|---|---|---|
| The service-account private key leaks through a log line, an error message, or an accidental serialization | Low | Critical | The handle is read in exactly one server-only module. The parsed credential is frozen and hardened on three independent axes: `privateKey` is a **non-enumerable** own property (excluded from `util.inspect`'s default output, `JSON.stringify`, `Object.keys`/`entries`, spread and `for...in`); a `nodejs.util.inspect.custom` hook renders a fixed redaction string — which is what actually protects `console.log`, since console formats through `util.inspect` and consults neither `toString` nor `toJSON`; and `toString`/`toJSON` cover coercion and explicit serialization. Every credential error reports the failing *structural rule* only, and the underlying `google-auth-library` error (which can embed the OAuth response) is discarded rather than wrapped. Regression tests assert non-disclosure through `util.inspect` (including `showHidden`), `util.format` in its `%s`/`%o`/`%O`/`%j` shapes, a real intercepted `console.log`, JSON serialization, string coercion, enumeration and spread — while confirming direct in-process access still works. |
| An oversized or unbounded provider response exhausts memory before it can be rejected | Low | Medium | Metadata and export bodies are consumed only through `readBoundedBody`, which checks the declared `content-length` as a cheap early-out and then counts bytes as the stream arrives, cancelling the moment the limit is crossed. The bound therefore holds when `content-length` is absent (chunked encoding) or understated, and no unbounded body is fully buffered or handed to `JSON.parse`. |
| A non-Doc Drive file is registered as a strategic source | Medium | Medium | The MIME check lives in the single central metadata read rather than only on the snapshot path, so registration — which reads metadata and nothing else — rejects a Sheet, Slide, PDF, folder or uploaded binary with HTTP 415 before anything is persisted. |
...
| 2026-08-09 | §9 updated after independent control review of PR #7 (Jira comment `11693`): credential hardening extended beyond `toString`/`toJSON` to non-enumerability plus a `util.inspect` hook; response bodies bounded by bytes read rather than declared `content-length`; MIME guard centralized so registration cannot persist a non-Doc. Decision itself unchanged. | Ricardo Souza |
